<template>
  <div
    class="sidebar bg-primary-900 text-greyscale-400 pt-8 flex flex-col justify-between"
    :class="isSidebarCollapsed ? 'collapsed ' : ''"
  >
    <div class="top flex flex-col flex-1">
      <div class="top-logo px-6 pb-6 flex flex-col">
        <div
          class="flex items-center mb-1 text-greyscale-200 corp-name"
        ...
        >
          ...
          <button
            ...
            class="responsive-uncollapser bg-primary-700 text-greyscale-white"
            @click="collapseBar()"
          >
      ...
      <div class="top-menu menu-list flex flex-col py-8 px-3 sato-l-l">
        ...
        <router-link
          :to="{ name: 'dashboard-collaborators' }"
          class="link"
      ...
      <div class="top-fav py-6 px-3 flex-1 text-greyscale-700 flex flex-col">
        <template v-if="!isSidebarCollapsed">
          <button
            class="fav-btn text-greyscale-500 flex items-center justify-between px-3"
            title="Favoris"
            @click="isFavOpen = !isFavOpen"
          >
            <i class="icon-favorite res-icon"></i>
            <span class="sato-l-m">Favoris</span>
            <i
              class="text-greyscale-400"
              :class="isFavOpen ? 'icon-caret-up ' : 'icon-caret-down '"
            ></i>
          </button>
    ...
    <div class="bottom menu-list flex flex-col">
      ...
      <div class="bottom-user">
        <div class="flex items-center">
          ...
          <span
            v-if="!isSidebarCollapsed"
            class="sato-p-s text-greyscale-white"
            >{{ getFullName }}</span
          >
        </div>
        <button>
