							...
							<div class="flex-1 table-container">
								<l-table :headers="progressionTableHeader" :items="member.templates" :showCheckBox="false" options>
									...
									<template #item-name="{ item }">
										<div class="text-greyscale-500 sato-l-s font-bold flex items-center">
											<i class="icon-tag mr-1"></i> <span class="upper">{{ item.name }}</span>
										</div>
									</template>
									<template #item-teamName="{ item }">
										<div class="text-greyscale-500 sato-l-s font-bold flex items-center">
											<i class="icon-work mr-1"></i><span class="upper">{{ item.teamName }}</span>
										</div>
									</template>
									<template #item-docs="{ item }">
										<div class="text-greyscale-500 sato-l-s font-bold">{{ item.docs.length }} documents</div>
									</template>
					...
					<div class="flex-1 table-container">
						<l-table :headers="documentTableHeaders" :items="member.documents" :showCheckBox="false" options>
							<template #item-name="{ item }">
								<div class="flex items-center">
									<div
										...
										:class="$options.status[item.status].textColor + ' ' + $options.status[item.status].bgColor"
									>
										<i :class="item.status === 'good' ? 'icon-check' : 'icon-document'" style="font-size: 16px"></i>
									</div>
							...
							<template #item-templateName="{ item }">
								<div class="text-greyscale-500 sato-l-s font-bold flex items-center">
									<i class="icon-tag mr-1"></i><span class="upper">{{ item.templateName }}</span>
								</div>
							</template>
							<template #item-date="{ item }">
								<div class="text-greyscale-500 sato-l-s font-bold flex items-center">
									<i class="icon-tag mr-1"></i><span class="upper">{{ formatDate(item.date) }}</span>
								</div>
							</template>
							<template #item-status="{ item }">
								<div class="sato-l-s font-bold" :class="$options.status[item.status].textColor">
									<i :class="$options.status[item.status].icon" class="mr-1"></i>{{ $options.status[item.status].str }}
								</div>
							</template>
