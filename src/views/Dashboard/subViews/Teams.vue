  ...
  >
    <div class="top-bar flex flex-col">
      ...
      <div class="sub-bar flex justify-between mt-10">
        <nav-bar
          v-model="subView"
          :items="['templates', 'membres', 'onboarding en cours']"
        />
        <router-link
        ...
        <role-selection
          v-else-if="subView === 'membres'"
          v-model="membersGlobalPermissionModel"
      ...
      <div class="sub-view-drawer flex-1 mt-6">
        <div v-if="subView === 'templates'" class="template-container">
          <div class="template-wrapper pb-4">
            <template-card
              v-for="(template, i) in teamData.templates"
              :key="template.id"
              :infos="template"
              :position="i + 1"
            />
            ...
            >
              <span class="text-primary-700 sato-l-m">
                Créer mon premier template <i class="icon-add ml-2"></i>
              </span>
            </router-link>
          </div>
        </div>
        <div v-else-if="subView === 'membres'" class="template-container">
            ...
            <template #item-lastActivity="{ item }">
              <div class="text-greyscale-500 flex items-center">
                ...
                <span class="sato-l-s font-bold"
                  >{{ formatLastActivity(item.lastActivity) }} Jours</span
                >
              </div>
            </template>
          ...
          <span v-else class="text-greyscale-700 sato-l-m font-bold"
            >Aucun membre pour le moment.</span
          >
        </div>
        <div
          v-else-if="subView === 'onboarding en cours'"
...
export default {
  ...
  beforeMount() {
    ...
    this.teamData = {
      ...
      isFav: false,
      members: [
        getUserInformation("ajzge"),
        getUserInformation("ajzgazee"),
        getUserInformation("ajzgecxvx"),
      ],
      templates: [
        {
          id: "plop",
