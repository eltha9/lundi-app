<template>
	<router-link
		:to="{ name: 'dashboard-templates', params: { id: template.id } }"
	...
	>
		...
		<div class="bottom-card flex justify-between items-center">
			...
			<div class="flex items-center font-bold text-greyscale-500 sato-l-s ml-24">
	...
	export default {
		...
		computed: {
			lastUpdate() {
				...
				const templateStamp = parseInt(new Date(this.template.lastUpdate).getTime() / 1000);
